...
def scheduler(quiet=True):
    ...
    while True:
        ...
        # not quiet and print(len(hosts.hosts),hosts.hosts)
        if len(hosts.hosts) > 0 :
            nextHostToTest = hosts.findNextHostToTest()
            host = hosts.getHost(nextHostToTest)
            not quiet and print(host["address"])
            hostTests = hosts.getHostTests(nextHostToTest)
            hostTests = doTests(hostTests,host,quiet)
            # not quiet and print("hostTests:",hostTests)
            hosts.updateHostTests(hostTests)
        time.sleep(5)

def doTests(hostTests,host,quiet=True):
    if "ping" in host:
        ping = host["ping"]
